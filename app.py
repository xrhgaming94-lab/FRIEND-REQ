...
# -----------------------------
# External JWT Token Generation Function
# -----------------------------
def get_token_from_external_api(uid, password):
    ...
    try:
        ...
        if data and 'token' in data:
            ...
            try:
                ...
                return None, "Token received but invalid format"
            
            return token, None
        else:
            error_msg = data.get('message', 'Token not found in response')
            return None, f"External API error: {error_msg}"
            
    except requests.RequestException as e:
        return None, f"External API request failed: {str(e)}"
    except ValueError as e:
        return None, f"Invalid JSON response from external API: {str(e)}"
    except Exception as e:
        return None, f"Unexpected error from external API: {str(e)}"

# -----------------------------
# Fallback JWT Generation (if external API fails)
# -----------------------------
def get_token_from_uid_password(uid, password):
    """Get JWT token using external API first, fallback to local method"""
    
    # Pehle external API try karo
    token, error = get_token_from_external_api(uid, password)
    
    if token:
        print(f"Token obtained from external API for UID: {uid}")
        return token, None
    
    print(f"External API failed: {error}. Trying local method...")
    
    # Agar external fail ho jaye toh local method try karo
    try:
...
def oauth_guest():
    """Get token using UID and password - NOW USING EXTERNAL API FIRST"""
    uid = request.args.get('uid')
    password = request.args.get('password')
    ...
    return jsonify({
        ...
        "uid": uid,
        "author_uid": author_uid,
        "source": "external_api"  # To track token source
    })

@app.route('/token/external', methods=['GET'])
def external_token_only():
    """Only use external API for token generation"""
    uid = request.args.get('uid')
    password = request.args.get('password')
